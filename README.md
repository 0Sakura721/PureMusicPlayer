...
- **随机播放全部**：曲库顶栏「随机」一键打乱全部歌曲并开始播放（借鉴 Auxio 的播放/随机入口），不改动当前播放模式记忆
- **续播更可靠**：播放中每约 15 秒持久化一次进度，即便被系统强杀也能恢复到近似位置；应用重启后从迷你播放器点播放可正确续播到上次位置（修复此前对未准备播放器调用 `start()` 的崩溃）
...
- **歌词**：优先匹配同目录同名 `.lrc`，其次读取文件内嵌歌词——FLAC / OGG（Vorbis Comment `LYRICS`/`UNSYNCEDLYRICS`）、MP3（ID3v2 `USLT`）、M4A / MP4（`©lyr` 原子）、WAV（RIFF LIST/INFO 的 `lyr`/`ILT` 块）；当前行高亮 + 平滑滚动；无时间戳的纯文本歌词逐行兜底展示。解析为纯 Kotlin 字节解析，零额外依赖
  - **双语/翻译自动识别**：相同时间戳的相邻行自动合并为「原文 + 译文」（覆盖中英、日英、韩英等任意语言对）；单句内用 `/ ｜ | ／ 、` 连接且两侧脚本不同（如中日韩表意文字 vs 拉丁字母）也会拆为译文行，避免误拆 "Verse 1 / 2"
  - **防溢出**：长歌词按视图宽度自动换行，译文单独成行显示，不再超出屏幕两侧
- **可视化**：`Visualizer` 同时采集 FFT 与波形数据，纯 `Canvas` 绘制三种样式——**条形**（频谱柱）、**圆形**（环绕径向频谱）、**波形**（居中音波线）；可在设置中开关与切换样式，并提供**实时预览**即时看到效果
- **音乐目录**：在「设置 → 曲库」用系统目录选择器限定仅扫描指定文件夹（SAF 文档树 + 持久化权限，重启后依然有效），清空即恢复扫描整个媒体库
- **明暗主题**：设置中可选 **跟随系统 / 浅色 / 深色 / 纯黑（AMOLED）**，基于 `AppCompatDelegate` 即时切换并记忆；纯黑模式背景为 `#000000`，在 OLED 屏上更省电
...
- **收藏（喜欢）**：播放页 / 列表 / 迷你栏一键 ♥ 收藏；曲库新增「收藏」标签页，用 `SharedPreferences` 持久化，重启后依然保留
- **睡眠定时器**：支持 15 / 30 / 45 / 60 分钟或「播完当前歌曲后停止」，播放页顶部实时显示倒计时；到点先渐隐音量再暂停，避免突兀
- **耳机/蓝牙拔出自动暂停**：断开有线耳机或蓝牙音频时自动暂停（设置可开关）
- **倍速播放**：播放页速度按钮在 0.5× / 0.75× / 1.0× / 1.25× / 1.5× / 2.0× 六档循环，偏好记忆；基于原生 `MediaPlayer.setPlaybackParams`（API 23+），零额外依赖
