...
- **歌词（folia 风）**：优先匹配同目录同名 `.lrc`，其次读取文件内嵌歌词——FLAC / OGG（Vorbis Comment `LYRICS`/`UNSYNCEDLYRICS`）、MP3（ID3v2 `USLT`）、M4A / MP4（`©lyr` 原子）、WAV（RIFF LIST/INFO 的 `lyr`/`ILT` 块）；当前行高亮 + 平滑滚动；无时间戳的纯文本歌词逐行兜底展示。解析为纯 Kotlin 字节解析，零额外依赖
